...
function sendAnalytics(metrics) {
    navigator.sendBeacon && navigator.sendBeacon(analyticsSettings.restUrl, JSON.stringify(metrics))
}

function sendBuffer() {
    sending = !0, 0 < buffer.length && (sendAnalytics({
        type: "multi",
        metrics: buffer
    }), buffer = []), sending = !1
}
...
function webVitalsReport({
    ...
    ("0" === analyticsSettings.multiMetrics ? sendAnalytics : bufferizeAnalytics)(analytics)
}

function performanceReport(timing, type) {
    let start = 0 < timing.startTime ? timing.startTime : 0;
    start = 0 < timing.workerStart ? timing.workerStart : start;
    let analytics = {
        type: type,
    ...
    0 < timing.secureConnectionStart ? analytics.metrics.push({
        name: "tcp",
        ...
        name: "redirects",
        value: timing.redirectCount || 0
    }), 0 < timing.transferSize ? analytics.metrics.push({
        name: "wait",
...
function webVitalsObserve() {
    getCLS(webVitalsReport), getFID(webVitalsReport), getLCP(webVitalsReport), getTTFB(webVitalsReport), getFCP(webVitalsReport)
}

function navigationObserve(list, observer) {
    0 < list.getEntriesByType("navigation").length && performanceReport(list.getEntriesByType("navigation")[0], "navigation")
}

function resourceObserve(list, observer) {
    0 < list.getEntriesByType("resource").length && list.getEntriesByType("resource").forEach(function(timing) {
        ("xmlhttprequest" !== timing.initiatorType && "beacon" !== timing.initiatorType && "other" !== timing.initiatorType || "1" !== analyticsSettings.smartFilter || !(0 < timing.name.indexOf("/beacon") || 0 < timing.name.indexOf("/livelog"))) && !excluded.includes(timing.name) && analyticsSettings.sampling >= getRandomArbitrary(1, 1e3) && (excluded.push(timing.name), performanceReport(timing, "resource"))
    })
}
try {
    webVitalsObserve();
    ...
    resourceObserver.observe({
        entryTypes: ["resource"]
    }), "1" === analyticsSettings.multiMetrics && (document.addEventListener("visibilitychange", function() {
        "hidden" !== document.visibilityState || sending || sendBuffer()

google.maps.__gjsload__('geometry', function(_) {
    var ioa = function(a, b) {
        ...
        joa = function(a, b, c, d, e) {
            if (!d) {
                ...
                if (!e) return e = Math.sin(_.ll(a.lat())), e = Math.log((1 + e) / (1 - e)) / 2, b = Math.sin(_.ll(b.lat())), _.ml(2 * Math.atan(Math.exp(e + c * (Math.log((1 + b) / (1 - b)) / 2 - e))) - Math.PI / 2);
                a = e.fromLatLngToPoint(a);
                b = e.fromLatLngToPoint(b);
                return e.fromPointToLatLng(new _.Do(a.x + c * (b.x - a.x), a.y + c * (b.y - a.y))).lat()
            }
            e = _.ll(a.lat());
            a = _.ll(a.lng());
            d = _.ll(b.lat());
            b = _.ll(b.lng());
            c = _.ll(c);
            return _.km(_.ml(Math.atan2(Math.sin(e) *
            ...
            for (let r = 0, t = g.getLength(); r < t; ++r) {
                ...
                for (let C = 0; C < y; ++C) {
                    ...
                    const J = Math.max(k, m),
                        H = Math.min(k, m);
                    if (k = Math.abs(_.km(k - m, -180, 180)) <= 1E-9 && (Math.abs(_.km(k - d, -180, 180)) <= c || Math.abs(_.km(m - d, -180, 180)) <= c)) {
                        k = a.lat();
                    ...
                    if (J - H > 180 ? d + c >= J || d - c <= H : d + c >= H && d - c <= J)
                        if (h = joa(h, F, d, f, b), Math.abs(h - a.lat()) < c) return !0
                }
            }
            return !1
        },
        ...
        qoa = function(a, b) {
            ...
            const c = Array(4),
                ...
                f = Array(3);
            ooa(a[a.length - 1], f);
            for (let v = 0; v < a.length; ++v) ooa(a[v], d), rD(f, d, c), poa(c, e, e), [f[0], f[1], f[2]] = d;
        ...
        rD = function(a, b, c) {
            ...
            if (e !== 0) b = Math.sqrt(e), c[0] = m / b, c[1] = p / b, c[2] = f / b, c[3] = d / b;
                ...
                for (f = 1; f < m.length; ++f) Math.abs(m[f]) < Math.abs(m[p]) && (p = f);f = [0, 0, 0];f[p] = 1;m = [m[1] * f[2] - m[2] * f[1], m[2] * f[0] - m[0] * f[2], m[0] * f[1] - m[1] * f[0]];p = Math.hypot(...m);m = [m[0] / p, m[1] / p, m[2] / p];p = Array(4);rD(a, m, p);a = Array(4);rD(m, b, a);poa(a, p, c)
            }
        },
        sD = class {};
        ...
        for (let k = 0, m = e.getLength(); k < m; ++k) {
            ...
            for (let r = 0, t = p.getLength(); r < t; ++r) {
                ...
                const y = _.km(w.lng(), -180, 180),
                    C = Math.max(h, y);
                h = Math.min(h, y);
                (C - h > 180 ? c >= C || c < h : c < C && c >= h) && joa(v, w, c, d, f) < a.lat() && (g = !g)
            }
        }
        return g || loa(a, b)
    };
    ...
    tD.computeArea = function(a, b) {
        if (!(a instanceof _.xp || Array.isArray(a) || a instanceof _.ko || a instanceof _.Ep)) try {
            a = _.jo(a)
        } catch (c) {
            try {
                a = new _.Ep((0, _.Hda)(a))
            } catch (d) {
                throw _.Hm("Invalid path passed to computeArea(): " + JSON.stringify(a));
            }
        }
        b = b || 6378137;
        if (a instanceof _.Ep) {
            if (a.getRadius() === void 0) throw _.Hm("Invalid path passed to computeArea(): Circle is missing radius.");
            if (a.getRadius() < 0) throw _.Hm("Invalid path passed to computeArea(): Circle must have non-negative radius.");
            if (b < 0) throw _.Hm("Invalid radiusOfSphere passed to computeArea(): radiusOfSphere must be non-negative.");
            if (a.getRadius() > Math.PI * b) throw _.Hm("Invalid path passed to computeArea(): Circle must not cover more than 100% of the sphere.");
            return 2 * Math.PI * b ** 2 * (1 - Math.cos(a.getRadius() / b))
        }
        if (a instanceof _.ko) {
            if (b < 0) throw _.Hm("Invalid radiusOfSphere passed to computeArea(): radiusOfSphere must be non-negative.");
            if (a.si.lo > a.si.hi) throw _.Hm("Invalid path passed to computeArea(): the southern LatLng of a LatLngBounds cannot be more north than the northern LatLng.");
    ...
    tD.interpolate = function(a, b, c) {
        ...
        return new _.dn(_.ml(Math.atan2(a * Math.sin(d) + c * Math.sin(f), Math.sqrt(b * b + e * e))), _.ml(Math.atan2(e, b)))
    };
    ...
    tD.computeOffset = function(a, b, c, d) {
        ...
        return new _.dn(_.ml(Math.asin(g)), _.ml(a + Math.atan2(b * e * Math.sin(c), d - f * g)))
    };
    tD.computeHeading = function(a, b) {
        ...
        a = _.fn(b);
        b = _.gn(b) - d;
        return _.km(_.ml(Math.atan2(Math.sin(b) * Math.cos(a), Math.cos(c) * Math.sin(a) - Math.sin(c) * Math.cos(a) * Math.cos(b))), -180, 180)
    };
    var soa = {
        encoding: _.mv,
        spherical: tD,
